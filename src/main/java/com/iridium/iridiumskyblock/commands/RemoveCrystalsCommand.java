...
public class RemoveCrystalsCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length != 3) {
            sender.sendMessage(StringUtils.color(IridiumSkyblock.getConfiguration().prefix) + "/is removecrystals <player> <amount>");
                ...
                if (island != null) {
                    if (org.apache.commons.lang.StringUtils.isNumeric(args[2])) {
                        ...
                        if (amount <= island.getCrystals()) {
                            ...
                            sender.sendMessage(StringUtils.color(IridiumSkyblock.getMessages().removedcrystals.replace("%crystals%", args[2]).replace("%player%", player.getName()).replace("%prefix%", IridiumSkyblock.getConfiguration().prefix)));
                        } else {
                            sender.sendMessage(StringUtils.color(IridiumSkyblock.getMessages().playerNotEnoughCrystals.replace("%player%", player.getName()).replace("%prefix%", IridiumSkyblock.getConfiguration().prefix)));
                        }
                    } else {
